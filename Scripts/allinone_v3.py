...
def convert_json_to_excel(json_file, excel_file):
    ...
    try:
        ...
        # JSON'daki "ports" ve diğer verileri işlemek için DataFrame oluşturun
        all_ports_data = []
        ...
        df = pd.DataFrame(all_ports_data)
        df.to_excel(excel_file, index=False)
        logging.info(f"JSON dosyası Excel'e dönüştürüldü: {excel_file}")
    except Exception as e:
        logging.error(f"JSON'u Excel'e dönüştürme sırasında hata: {e}")

# JSON dosyasını oluşturma ve çevirme işlemi
if __name__ == "__main__":
    fetch_network_data()  # JSON dosyasını oluştur
