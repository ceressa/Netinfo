...
def get_device_interfaces(hostname, token):
    ...
    try:
        ...
        logging.info(f"{hostname} aray�z verileri basariyla alindi.")
        return response.json()
    except requests.RequestException as e:
        logging.error(f"{hostname} aray�z verileri alinamadi: {e}")
        return None

# Sadece `is_up = True` olan portlari filtreleme ve Mbps'ye �evirme
...
def main():
    ...
    if not interface_data:
        logging.error(f"{hostname} i�in veri alinamadi, islem sonlandiriliyor.")
        return
