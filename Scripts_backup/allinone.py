...
input_file = "D:/INTRANET/Netinfo/Data/merged_statseeker_data.xlsx"
...
log_file = f"{log_dir}fetch_network_data_{datetime.now().strftime('%d%m%Y%H%M')}.log"
...
def fetch_device_data(hostname, bearer_token, retry=0):
    ...
    if retry < 2:
        logging.info(f"Yeniden deneniyor: {hostname} (Deneme {retry + 2}/3)")
...
def fetch_network_data():
    ...
    for _, row in switches.iterrows():
        result = process_device_data(row, bearer_token)
        if result:
            all_data[row['hostname']] = result
        else:
            failed_devices.append(row)

    # Başarısız olan cihazları tekrar dene
    for device in failed_devices:
        result = process_device_data(device, bearer_token)
        if result:
            all_data[device['hostname']] = result
