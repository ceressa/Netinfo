...
# SSL Uyarılarını Devre Dışı Bırakma
...
def fetch_bearer_token():
    ...
    try:
        ...
        if not token:
            logging.error("Bearer token alınamadı, response: %s", response.text)
            return None
        return token
    except requests.exceptions.SSLError as ssl_err:
        logging.error("SSL sertifikası doğrulama hatası: %s", ssl_err)
        return None
    except requests.exceptions.RequestException as req_err:
        logging.error("Token alımı sırasında hata oluştu: %s", req_err)
        return None
...
def save_to_excel(all_vlan_data):
    if all_vlan_data:
        ...
        logging.info(f"Tüm cihazların VLAN bilgileri {vlan_output_file} dosyasına kaydedildi.")
    else:
        logging.warning("Hiçbir cihaz için VLAN datası kaydedilmedi.")
...
def main():
    ...
    try:
        ...
        logging.error(f"Statseeker dosyası okunurken hata oluştu: {e}")
        return
