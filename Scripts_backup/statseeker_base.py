...
def fetch_data(url):
    ...
    try:
        ...
        response = requests.get(
            ...
            verify=False,
            timeout=60,
            proxies=None  # Proxy kullanma - direkt bağlantı
        )
...
def debug_noc_turkey_devices():
    ...
    try:
        response = requests.get(group_url, auth=(user, password), verify=False, timeout=60, proxies=None)
        if response.status_code == 200:
            data = response.json()
    ...
    try:
        response = requests.get(recent_device_url, auth=(user, password), verify=False, timeout=60, proxies=None)
        if response.status_code == 200:
            data = response.json()
            if 'data' in data and 'objects' in data['data'] and data['data']['objects']:
...
def debug_device_inventory_mismatch():
    ...
    for name, url in urls.items():
        try:
            response = requests.get(url, auth=(user, password), verify=False, timeout=60, proxies=None)
            if response.status_code == 200:
                data = response.json()
...
def debug_model_types():
    ...
    try:
        response = requests.get(inventory_url, auth=(user, password), verify=False, timeout=60, proxies=None)
        if response.status_code == 200:
            data = response.json()
            df = process_data(data)
