...
:: Remote baglantisini kontrol et
git ls-remote --exit-code %REMOTE% >nul 2>&1
if !errorlevel! neq 0 (
    echo [ERROR] Remote '%REMOTE%' baglantisi basarisiz.
    echo [INFO] Agi kontrol edin veya remote URL'i dogrulayin.
    goto :error_exit
)
...
if !HAS_UNSTAGED! neq 0 (
    ...
    git status --short
    echo.
    echo [SORU] Degisiklikleri otomatik commit edeyim mi?
    ...
    if /i "!COMMIT_CHOICE!"=="S" (
        ...
        git stash push -m "deploy-oncesi-stash-%date:~-4%%date:~3,2%%date:~0,2%"
        if !errorlevel! neq 0 (
    ...
    echo [WARNING] Stage edilmis ama commit edilmemis degisiklikler var!
    echo [INFO] Otomatik commit ediliyor...
    git commit -m "Deploy oncesi otomatik commit - %date% %time:~0,8%"
    if !errorlevel! neq 0 (
        echo [ERROR] Commit basarisiz.
        goto :error_exit
...
echo [INFO] '%CURRENT_BRANCH%' branch'i '%TARGET_BRANCH%' icine merge edilecek.
set "SOURCE_BRANCH=!CURRENT_BRANCH!"

:: Merge oncesi commit sayisini goster
...
if !errorlevel! neq 0 (
    ...
    echo [ERROR] Merge catismasi olustu.
    echo [INFO] Catismalari cozup tekrar calistirin:
    echo         git mergetool
    echo         git commit
    echo         deploy.bat
    echo.
...
:: 4 denemeye kadar retry (exponential backoff)
set "PUSH_OK=0"
for %%d in (0 2 4 8) do (
    if !PUSH_OK!==0 (
        if %%d gtr 0 (
            echo [INFO] %%d saniye bekleniyor...
        ...
        if !errorlevel!==0 (
            ...
            echo [WARNING] Push denemesi basarisiz. Tekrar deneniyor...
        )
    )
)
if !PUSH_OK!==0 (
    echo [ERROR] Push 4 denemeden sonra basarisiz.
    echo [WARNING] Deploy yerel olarak devam edecek, push'u sonra yapabilirsiniz.
) else (
...
if exist "%windir%\system32\inetsrv\appcmd.exe" (
    ...
    :: Uygulamanin dosyalari serbest birakmasi icin bekle
...
:: Once restore, sonra publish (daha net hata mesajlari icin)
dotnet restore
if !errorlevel! neq 0 (
...
if !errorlevel! neq 0 (
    ...
    echo [INFO] Hatalari inceleyin ve tekrar deneyin.
    goto :start_pool_and_exit
)
echo.
...
:: Publish dizininde Netinfo.dll var mi?
if exist "%PUBLISH_PATH%\Netinfo.dll" (
    echo [OK] Netinfo.dll mevcut.
    for %%f in ("%PUBLISH_PATH%\Netinfo.dll") do (
        echo [INFO] DLL boyutu: %%~zf bytes
        echo [INFO] DLL tarihi: %%~tf
    )
) else (
    echo [ERROR] Netinfo.dll bulunamadi! Publish basarisiz olabilir.
)

:: wwwroot/index.html var mi?
...
if defined DID_STASH (
    ...
    git stash pop
    if !errorlevel! neq 0 (
        echo [WARNING] Stash pop basarisiz. Manuel olarak calistirin: git stash pop
    ) else (
...
if exist "%windir%\system32\inetsrv\appcmd.exe" (
    "%windir%\system32\inetsrv\appcmd.exe" start apppool /apppool.name:"%APPPOOL_NAME%" >nul 2>&1
    echo [OK] App Pool baslatildi (eski surum ile).
)
goto :error_exit
